	...
	iScroll.prototype = {
		...
		_start: function (e) {
			...
			if (that.options.momentum) {
				...
				if (x != that.x || y != that.y) {
					...
					that.steps = [];
					that._pos(x, y);
					if (that.options.onScrollEnd) that.options.onScrollEnd.call(that, true);
				}
			}
		...
		_resetPos: function (time) {
			...
			if (resetX == that.x && resetY == that.y) {
				if (that.moved) {
					that.moved = false;
					if (that.options.onScrollEnd) that.options.onScrollEnd.call(that, false);		// Execute custom code on scroll end
				}
